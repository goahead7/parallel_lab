...
{
    std::atomic<unsigned> m_refs = std::atomic<unsigned>(2);
    unsigned m_r1, m_r2;
...
public:
    static auto create(unsigned* result_ptr, std::shared_ptr<control_state> parent_state)
    {
        return std::shared_ptr<control_state_fib_async>(new control_state_fib_async(result_ptr, parent_state));
    }
...
public:
    ...
    void run() override
    {
        ...
        {
            auto ctrl = control_state_fib_async::create(m_result_ptr, m_ctrl);

            auto task1 = std::make_unique<task_fib>(m_n - 1, ctrl, ctrl->result1_ptr());
            auto task2 = std::make_unique<task_fib>(m_n - 2, ctrl, ctrl->result2_ptr());
